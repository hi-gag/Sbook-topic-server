    ...
    const queuedPromises = bookmarks.docs.map((bookmark) => {
      return new Promise(async (resolve, reject) => {
        ...
        const response = await axios.get(
          `https://customsearch.googleapis.com/customsearch/v1?cx=006045920514796618854%3Ac71ixzdakt6&q=${query}&lr=lang_ko&key=AIzaSyDJ10zw0KLAOWzDP4YSKbAicTF_bGxP7ek`
        );
        const searchResults = response.data.items.slice(0, 3);

        const recommendPromises = searchResults.map(
          (result) =>
